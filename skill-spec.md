...
### Output: Unified `SKILL.md`

All skills are consolidated into a single **`SKILL.md`** file at the repository root.
This file is auto-generated by `generate_skills.py` and serves as the primary AI
skill document.

Individual skill cards within `SKILL.md` follow this structure:
...
`SKILL.md` is also automatically regenerated by the `Generate Skills` GitHub Actions
workflow when tutorials are updated on the main branch.
