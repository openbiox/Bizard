...
-   Dependent packages: `data.table`; `jsonlite`; `destiny`; `ggplotify`; `scatterplot3d`; `ggpubr`; `smoother`
...
if (!requireNamespace("smoother", quietly = TRUE)) {
  install.packages("https://cran.r-project.org/src/contrib/Archive/smoother/smoother_1.3.tar.gz")
}
