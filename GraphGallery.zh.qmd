...
// Smart Recommender Functions (Chinese)
let keywordsDataZh = null;

async function loadKeywordsZh() {
  if (!keywordsDataZh) {
    const response = await fetch('files/recommender_keywords_zh.json');
    keywordsDataZh = await response.json();
  }
  return keywordsDataZh;
}

function tokenizeZh(text) {
  // For Chinese, we need to handle both Chinese characters and English words
  // Split by spaces, punctuation, and also extract Chinese character sequences
    ...
    results.map(r => `
      <div class="recommender-card" style="display: flex; align-items: center; margin: 8px 0; padding: 12px; border: 1px solid #e0e0e0; border-radius: 4px; background: #fafafa;">
        ...
        <div style="flex: 1;">
          ...
          <p style="margin: 4px 0 0 0; color: #888; font-size: 0.8em;">匹配关键词: ${r.matchedKeywords.slice(0, 5).join(', ')}</p>
        </div>
      </div>
    `).join('');
}
