...
async function recommendChartsEn() {
  ...
  if (!input) {
    document.getElementById('recommender-results').innerHTML = '<p style="color: #666;">Please enter a description of your visualization goal.</p>';
    return;
  }
  ...
  resultsDiv.innerHTML = `<p style="font-weight: 600;">Matching Results (${results.length}):</p>` +
    results.map(r => `
      <div class="recommender-card" style="display: flex; align-items: center; margin: 8px 0; padding: 12px; border: 1px solid #e0e0e0; border-radius: 4px; background: #fafafa;">
        <img src="${r.chart.Image_url}" style="width: 80px; height: 80px; object-fit: cover; border-radius: 4px; margin-right: 12px;" onclick="showModal(this)">
        <div style="flex: 1;">
          <a href="${r.chart.Tutorial_url}" target="_blank" style="color: #007899; font-weight: 600; text-decoration: none;">${r.chart.Name}</a>
          <p style="margin: 4px 0 0 0; color: #666; font-size: 0.9em;">${r.chart.Description}</p>
          <p style="margin: 4px 0 0 0; color: #888; font-size: 0.8em;">Matched: ${r.matchedKeywords.slice(0, 5).join(', ')}</p>
        </div>
      </div>
    `).join('');
}
