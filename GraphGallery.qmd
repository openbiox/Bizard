...
async function loadKeywordsEn() {
  if (!keywordsDataEn) {
    try {
      const response = await fetch('files/recommender_keywords.json');
      if (!response.ok) {
        throw new Error(`Failed to load keywords: ${response.status}`);
      }
      keywordsDataEn = await response.json();
    } catch (error) {
...
function expandSynonyms(tokens, synonyms) {
  ...
  tokens.forEach(token => {
    ...
    if (synonyms[token]) {
      synonyms[token].forEach(syn => expanded.add(syn.toLowerCase()));
    }
...
function calculateScore(tokens, intents, text, chart) {
  ...
  if (tokens.some(t => chart.Type.toLowerCase().includes(t))) score += 3;
